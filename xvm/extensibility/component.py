...
from xvm._utils.os import friendly_filename_or_url
from xvm._utils.logging import get_logger
from xvm.boot_info import BootInfo
from xvm.cli_parser import parser, subparser
from xvm.config import BaseAppConfig, XvmAppConfig, XvmConfig
from xvm.constants import APP_IDENTIFIER_ETH1, SYNC_FULL
from xvm.initialization import initialize_data_dir, is_data_dir_initialized

if TYPE_CHECKING:
    from xvm.extensibility.trio import TrioIsolatedComponent  # noqa: F401
    from xvm.extensibility.asyncio import AsyncioIsolatedComponent  # noqa: F401

TReturn = TypeVar("TReturn")
logger = logging.getLogger('xvm.extensibility.component')
...
class BaseComponentAPI(ABC):
    ...
    def configure_parser(cls, arg_parser: ArgumentParser, subparser: _SubParsersAction) -> None:
        """
        Give the component a chance to amend the Xvm CLI argument parser.
        """
        ...
...
class BaseComponent(ComponentAPI):
    ...
    # name.
    logger = get_logger('xvm.components.BaseComponent')

    def __init__(self, boot_info: BootInfo) -> None:
        if not hasattr(self, 'name'):
            raise AttributeError(f"No name attribute defined for {self.__class__}")
        self.logger = get_logger(f'xvm.components.{self.name}')
...
class BaseIsolatedComponent(BaseComponent):
    """
    A :class:`~xvm.extensibility.component.BaseIsolatedComponent` runs in an isolated process
    ...
    def get_subprocess_kwargs(self) -> Optional[SubprocessKwargs]:
        ...
        if os.getenv('XVM_SINGLE_PROCESS_GROUP') == "1":
...
def _setup_standalone_component(
    ...
    if app_identifier == APP_IDENTIFIER_ETH1:
        app_cfg: Type[BaseAppConfig] = XvmAppConfig
    else:
    ...
    xvm_config = XvmConfig.from_parser_args(args, app_identifier, (app_cfg,))
    xvm_config.xvm_root_dir.mkdir(exist_ok=True)
    if not is_data_dir_initialized(xvm_config):
        initialize_data_dir(xvm_config)
    boot_info = BootInfo(
        args=args,
        xvm_config=xvm_config,
        min_log_level=None,
        logger_levels=None,
        profile=False,
    )
...
async def _run_eventbus_for_component(
    ...
    from xvm.extensibility.trio import TrioIsolatedComponent
    from xvm.extensibility.asyncio import AsyncioIsolatedComponent
    ...
    xvm_config = component._boot_info.xvm_config
    conn_config = ConnectionConfig.from_name(
        component.get_endpoint_name(), xvm_config.ipc_dir)
...
def run_asyncio_xvm_component(component_type: Type['AsyncioIsolatedComponent']) -> None:
    import asyncio
    from xvm._utils.asyncio_utils import wait_first
...
def run_trio_xvm_component(component_type: Type['TrioIsolatedComponent']) -> None:
