...
class XvmConfigParams(TypedDict):
    network_id: int

    xvm_root_dir: Optional[Path]
...
def construct_xvm_config_params(
        args: argparse.Namespace) -> XvmConfigParams:
    return cast(XvmConfigParams, dict(_construct_xvm_config_params(args)))
...
def _construct_xvm_config_params(
        args: argparse.Namespace
) -> Iterable[Tuple[str, Union[int, str, bytes, Path, Tuple[str, ...]]]]:
    """
    Helper function for constructing the kwargs to initialize a XvmConfig object.
    """
    yield 'xvm_tmp_root_dir', args.xvm_tmp_root_dir
    if args.xvm_tmp_root_dir:
        yield 'xvm_root_dir', Path(tempfile.gettempdir()) / Path(_random_symbol_of_length(4))
    elif args.xvm_root_dir is not None:
        yield 'xvm_root_dir', args.xvm_root_dir
