...
from xvm.vm import opcode_values
...
class BlockHeaderAPI(MiningHeaderAPI, BlockHeaderSedesAPI):
    """
    A class derived from :class:`~xvm.abc.MiningHeaderAPI` to define a block header
    after it is sealed.
    """
...
class XvmBlockHeaderAPI(BlockHeaderAPI):

    @property
    @abstractmethod
    def xvm_block_hash(self) -> Hash32:
        """
...
class TransactionFieldsAPI(ABC):
    ...
    @property
    @abstractmethod
    def xvm_sender(self) -> Address:
        ...


    @property
    @abstractmethod
    def xvm_txhash(self) -> Hash32:
        ...
...
class HeaderDatabaseAPI(ABC):
    ...
    def __init__(self, db: AtomicDatabaseAPI) -> None:
        """
        Instantiate the database from an :class:`~xvm.abc.AtomicDatabaseAPI`.
        """
        ...
...
class ChainDatabaseAPI(HeaderDatabaseAPI):
    ...
    :class:`~xvm.abc.HeaderDatabaseAPI`.
    """
    ...
    def persist_block(
        ...
        .. warning::
            ...
            :meth:`xvm.abc.ChainDatabaseAPI.persist_unexecuted_block` to persist blocks
    ...
    def persist_unexecuted_block(
        ...
        **not** validate if state transitions are valid.
        ...
        transactions which :meth:`xvm.abc.ChainDatabaseAPI.persist_block` in contrast
...
class MemoryAPI(ABC):
    """
    A class representing the memory of the :class:`~xvm.abc.VirtualMachineAPI`.
    """
...
class StackAPI(ABC):
    """
    A class representing the stack of the :class:`~xvm.abc.VirtualMachineAPI`.
    """
    ...
    def pop1_bytes(self) -> bytes:
        ...
        Raise `xvm.exceptions.InsufficientStack` if the stack was empty.
        """
        ...
    ...
    def pop_any(self, num_items: int) -> Tuple[Union[int, bytes], ...]:
        ...
        Raise `xvm.exceptions.InsufficientStack` if there are not enough items on
        the stack.
...
class ComputationAPI(
    ...
    def error(self) -> VMError:
        """
        Return the :class:`~xvm.exceptions.VMError` of the computation.
    ...
    def error(self, value: VMError) -> None:
        """
        Set an :class:`~xvm.exceptions.VMError` for the computation.
        """
    ...
    def extend_memory(self, start_position: int, size: int) -> None:
        ...
        bytes in length.  Raise `xvm.exceptions.OutOfGas` if there is not enough
    ...
    def consume_gas(self, amount: int, reason: str) -> None:
        ...
        Raise `xvm.exceptions.OutOfGas` if there is not enough gas remaining.
        """
        ...
...
class AccountDatabaseAPI(ABC):
    ...
    def lock_changes(self) -> None:
        ...
        See :meth:`xvm.abc.AccountStorageDatabaseAPI.lock_changes` for
...
class TransactionExecutorAPI(ABC):
    ...
    def __call__(self, transaction: SignedTransactionAPI) -> "ComputationAPI":
        """
        Execute the ``transaction`` and return a :class:`xvm.abc.ComputationAPI`.
        """
        ...
    ...
    def build_evm_message(self, transaction: SignedTransactionAPI) -> MessageAPI:
        """
        Build and return a :class:`~xvm.abc.MessageAPI` from the given ``transaction``.
        """
        ...
...
class StateAPI(ConfigurableAPI):
    ...
    Each :class:`~xvm.abc.VirtualMachineAPI` must be configured with a subclass of the
    :class:`~xvm.abc.StateAPI`.

      .. note::

        Each :class:`~xvm.abc.StateAPI` class must be configured with:

        - ``computation_class``: The :class:`~xvm.abc.ComputationAPI` class for
          vm execution.
        - ``transaction_context_class``: The :class:`~xvm.abc.TransactionContextAPI`
    ...
    def get_account_db_class(cls) -> Type[AccountDatabaseAPI]:
        """
        Return the :class:`~xvm.abc.AccountDatabaseAPI` class that the
        state class uses.
        """
        ...
    ...
    def get_transaction_context_class(cls) -> Type[TransactionContextAPI]:
        """
        Return the :class:`~xvm.vm.transaction_context.BaseTransactionContext` class
    ...
    def get_transaction_context(
        ...
        """
        Return the :class:`~xvm.abc.TransactionContextAPI` for the given ``transaction``
        """
        ...

class VirtualMachineAPI(ConfigurableAPI):
    """
    The :class:`~xvm.abc.VirtualMachineAPI` class represents the Chain rules for a
    specific protocol definition such as the Frontier or Homestead network.

      .. note::

        Each :class:`~xvm.abc.VirtualMachineAPI` class must be configured with:

        - ``block_class``: The :class:`~xvm.abc.BlockAPI` class for blocks in this
            VM ruleset.
        - ``_state_class``: The :class:`~xvm.abc.StateAPI` class used by this
            VM for execution.
    ...
    def apply_transaction(
        ...
        :func:`~xvm.vm.state.State.apply_transaction` with some extra
        orchestration logic.
    ...
    def create_execution_context(
        ...
        """
        Create and return the :class:`~xvm.abc.ExecutionContextAPI`` for the given
    ...
    def get_block_class(cls) -> Type[BlockAPI]:
        """
        Return the :class:`~xvm.rlp.blocks.Block` class that this VM uses for blocks.
        """
        ...
    ...
    def validate_header(
        ...
        """
        :raise xvm.exceptions.ValidationError: if the header is not valid
        """
        ...
...
class HeaderChainAPI(ABC):
    """
    Like :class:`xvm.abc.ChainAPI` but does only support headers, not entire blocks.
    """
...
class ChainAPI(ConfigurableAPI):
    ...
    def get_chaindb_class(cls) -> Type[ChainDatabaseAPI]:
        """
        Return the class for the used :class:`~xvm.abc.ChainDatabaseAPI`.
        """
        ...
    ...
    def get_block_by_hash(self, block_hash: Hash32) -> BlockAPI:
        ...
        :raise xvm.exceptions.HeaderNotFound: if the header is missing
        :raise xvm.exceptions.BlockNotFound: if any part of the block body is missing
        """
        ...
    ...
    def validate_chain_extension(self, headers: Tuple[BlockHeaderAPI, ...]) -> None:
        ...
        :meth:`~xvm.abc.ChainAPI.validate_chain`, it is an additional API to call at a
...
class MiningChainAPI(ChainAPI):
    """
    Like :class:`~xvm.abc.ChainAPI` but with APIs to create blocks incrementally.
    """
    ...
    def mine_block(self, *args: Any, **kwargs: Any) -> BlockAPI:
        ...
        See VM. :meth:`~xvm.vm.base.VM.mine_block`
        """
        ...
