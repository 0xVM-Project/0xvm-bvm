...
from xvm.constants import MAIN_EVENTBUS_ENDPOINT
from xvm.events import AvailableEndpointsUpdated, EventBusConnected


class BaseEventBusService(Service):
    logger = logging.getLogger('xvm.extensibility.event_bus.EventBusService')
    ...
    async def run(self) -> None:
        async with self.EndpointType.serve(self._connection_config) as endpoint:
            ...
            main_endpoint_config = ConnectionConfig.from_name(
                MAIN_EVENTBUS_ENDPOINT, self._xvm_config.ipc_dir
            )
