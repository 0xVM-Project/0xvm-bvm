...
from xvm.db.atomic import AtomicDBWriteBatch
from xvm.db.backends.base import BaseAtomicDB
from xvm.db.diff import DBDiff

from xvm._utils.ipc import wait_for_ipc
from xvm._utils.socket import BufferedSocket, IPCSocketServer
...
class DBManager(IPCSocketServer):
    ...
    """
    logger = logging.getLogger('xvm.db.manager.DBManager')
...
class AtomicBatch(AtomicDBWriteBatch):
    ...
    """
    logger = logging.getLogger("xvm.db.manager.AtomicBatch")
...
class DBClient(BaseAtomicDB):
    logger = logging.getLogger('xvm.db.client.DBClient')
...
def _run() -> None:
    from xvm.db.backends.level import LevelDB
    from xvm.db.chain import ChainDB
    from xvm.cli_parser import parser
    from xvm.config import XvmAppConfig, XvmConfig
    from xvm.constants import APP_IDENTIFIER_ETH1
    from xvm.initialization import (
        initialize_data_dir,
    ...
    xvm_config = XvmConfig.from_parser_args(args, APP_IDENTIFIER_ETH1, (XvmAppConfig,))
    xvm_config.xvm_root_dir.mkdir(exist_ok=True)
    if not is_data_dir_initialized(xvm_config):
        initialize_data_dir(xvm_config)

    with xvm_config.process_id_file('database'):
        app_config = xvm_config.get_app_config(XvmAppConfig)
        ensure_xvm_dirs(app_config)
        ...
        with manager.run(xvm_config.database_ipc_path):
            try:
