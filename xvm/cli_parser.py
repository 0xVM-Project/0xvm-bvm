...
parser = argparse.ArgumentParser(description='Xvm')

#
# subparser for sub commands
#
# Components may add subcommands with a `func` attribute
# to gain control over the main Xvm process
subparser = parser.add_subparsers(dest='subcommand')

#
# Argument Groups
#
xvm_parser = parser.add_argument_group('core')
...
# Xvm Globals
#
xvm_parser.add_argument('--version', action='version', version=__version__)
xvm_parser.add_argument(
    '--xvm-root-dir',
    help=(
        "The filesystem path to the base directory that xvm will store it's "
        "information.  Default: $XDG_DATA_HOME/.local/share/xvm"
    ),
)
xvm_parser.add_argument(
    '--port',
    ...
    default=30303,
    help=(
        "Port on which xvm should listen for incoming p2p/discovery connections. Default: 30303"
    ),
)
xvm_parser.add_argument(
    '--xvm-tmp-root-dir',
    ...
    default=False,
    help=(
        "If this flag is set, xvm will launch with a temporary root"
